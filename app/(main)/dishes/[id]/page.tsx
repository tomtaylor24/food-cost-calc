    ...
    const load = async () => {
      try {
        ...
        const [dishResponse, ingredientsResponse] = await Promise.all([
          fetch(`/api/dishes/${params.id}`, { headers }),
          fetch("/api/ingredients", { headers })
        ])
        const [dishJson, ingredientsJson] = await Promise.all([
          dishResponse.json(),
          ingredientsResponse.json()
        ])
        if (!dishResponse.ok) {
          toast.error(dishJson.message)
          router.push("/dishes")
          return
        }
        const singleItem = dishJson.dish as DishDetail
        reset({
        ...
        if (ingredientsResponse.ok) {
          ...
          toast.error(ingredientsJson.message)
        }
        setIsLoading(false)
      } catch {
      ...
      <div className="container">
        ...
        <div className="pageMain">
          ...
          <div className="pageActions">
            <button form="dishForm" className="btn formSubmit" disabled={isSubmitting || isLoading} aria-busy={isSubmitting}>
              {isSubmitting && <span className="btnSpinner" aria-hidden="true" />}変更を保存
            </button>
            <button className="formDelete" type="button" onClick={handleDelete} disabled={isSubmitting || isLoading}>商品を削除</button>
          </div>
        </div>
